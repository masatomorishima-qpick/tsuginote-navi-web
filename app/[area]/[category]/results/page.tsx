...
function createEphemeralSessionId(area: string, category: string): string {
  ...
  return `${area}-${category}-${randomPart}`;
}
...
function buildSessionId(
  ...
  // 固定フォールバックをやめて、そのリクエスト専用の一時sessionIdを採用
...
export default async function ResultsPage({
  ...
  const offices = await getPublishedOffices();
  ...
  return (
    <main className="min-h-screen bg-slate-50 text-slate-900">
      <SurveySubmitTracker
        ...
        category={category}
        answers={answers}
        resultCount={result.offices.length}
      />
      ...
      <header className="border-b border-slate-200 bg-white">
        <div className="mx-auto flex max-w-6xl items-center justify-between px-4 py-4 sm:px-6">
          ...
          <BackButton
            fallbackHref={`/${area}/${category}/start`}
            label="戻る"
