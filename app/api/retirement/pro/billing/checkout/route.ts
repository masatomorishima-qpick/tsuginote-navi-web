 ...
 *
 * 老後のお金の受け取りシミュレーション（有料版・19,800円税込・一回払い）の
 ...
 *   ★★★2026-09-15・決め1258 …… ★**画面5-6の「有料版購入」は、この口を直に呼びます**
 *     （`components/retirement/pro/ProApp.tsx` の `onKau`）。★前は `/retirement/pro/buy` へ飛ばしていました。
 *     ★`buy` の頁は、★**この口が 404 のときの行き先**として残っています（★同じ栓で中身が分かれます）。
 */
...
export async function POST(req: Request) {
  ...
  //   ★★★2026-09-15・決め1258（戦術Cowork `senjutsu_20260915j.md` 3-4）。★前はここに
  //     「Production には入れません（入れるのは A-2）。Preview にだけ '1' を入れて、当てます。」
  //     と書いていました。★★これは 2026-09-02 の姿で、★**いまが、その A-2 の回です**。
  //   ★★いまの決め …… **Preview に '1' を入れて当ててから、森嶋さんのお決めで本番へ。**
  //     ★★★**どの入れ物に入れるかは、森嶋さんが Vercel でお決めになることです**（★こちらからは見えません）。
  //     ★repo の中には、この名前は**この本の3か所だけ**です（`vercel.json` にも 0か所）。
