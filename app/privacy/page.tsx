    ...
    <div className="flex min-h-screen flex-col bg-slate-50 text-slate-900">
      ...
      <main className="flex-1">
        <section className="mx-auto max-w-4xl px-4 py-10 sm:px-6 sm:py-14">
          ...
          <p className="mt-3 text-sm leading-7 text-slate-600 sm:text-base">
            第1〜12条は候補案内機能を前提とした共通の内容です。
            デジタル資産機能は第13条に、資産づくり診断は第14条に、
            退職金とiDeCoの受け取り方シミュレーション（有料版）は第15条に、それぞれ個別事項を定めます。
            第1〜12条と各条で異なる内容が定められている場合、その機能については各条が優先します。
          </p>
          ...
          <div className="mt-10 space-y-10 text-sm leading-7 text-slate-700 sm:text-base">
            <section>
              ...
              <p className="mt-3">
                当サイトでは、候補案内機能のアンケート回答にあたり、氏名、メールアドレス、電話番号その他の個人を特定できる情報を取得・保存しません。
                デジタル資産機能、資産づくり診断、退職金とiDeCoの受け取り方シミュレーション（有料版）で取得する情報は、第13条〜第15条に定めます。
              </p>
              <p className="mt-3">
                また、自由記述欄は、任意でご意見・ご感想をご入力いただく欄のみを設けており、氏名・連絡先など個人が分かる情報は入力しないようご案内しています（第14条）。個人情報の入力を前提とした設計にはしていません。
              </p>
            </section>
            ...
            <section>
              ...
              <p className="mt-3">
                ...
                候補案内機能では取得・保存を行いません。
                第13条〜第15条に定める機能で取得する情報の保存期間は、各条に定めます。
              </p>
            </section>
            ...
            <section>
              ...
              <p className="mt-3">
                当サイトでは、候補案内機能について、個人を識別できる情報を保有していないため、
                個人情報保護法上の開示、訂正、利用停止、削除等の対象となる情報を保有しない場合があります。
                第13条〜第15条に定める機能で保有する情報については、各条に定める方法でご請求いただけます。
              </p>
              <p className="mt-3">
            ...
            <section>
              ...
              <ul className="mt-3 list-disc space-y-2 pl-6">
                ...
                <li>任意でご回答いただいた追加項目（資産のおおまかな内訳・目的・世帯構成・すでに着手している対策など。すべて任意・タップ選択のみ）</li>
                <li>任意でご入力いただいたご意見・ご感想（自由記述）。個人が分かる内容は入力しないようご案内していますが、入力された場合も、統計・サービス改善の目的にのみ利用します。</li>
                <li>参照元・流入計測情報（リファラー、utm パラメータ 等）、利用環境情報（ブラウザの種類 等）</li>
                <li>おおまかな地域（都道府県レベル）。アクセス時のIPアドレスをもとにホスティング事業者（Vercel）が推定した地域区分を、統計・分析の目的で記録します。<strong>IPアドレスそのものは保存せず</strong>、記録するのは都道府県相当の地域区分（および国）のみです。市区町村より細かい情報や正確な位置情報は取得・記録しません。個人の特定を目的とせず、匿名の診断記録の一部として保存します。</li>
              </ul>
              <p className="mt-3">
            ...
            <section>
              <h2 className="text-xl font-semibold text-slate-900">
                15. 退職金とiDeCoの受け取り方シミュレーション（有料版）における情報の取扱い
              </h2>
              <p className="mt-3">
                退職金とiDeCoの受け取り方シミュレーション（/retirement/pro）は、登録不要でご利用いただけます。
                無料版では、ご入力いただいた内容を保存しません。
                有料版をご購入いただいた場合に限り、以下の情報を取得・保存します。
                第1条〜第10条で「取得・保存しない」「個人を識別できる情報を保有しない」旨が記載されている箇所は、
                候補案内機能を前提としており、有料版については本条が優先します。
              </p>
              <h3 className="mt-6 text-base font-semibold text-slate-900">
                15-1. 取得する情報
              </h3>
              <ul className="mt-3 list-none space-y-1">
                <li>・メールアドレス（決済時に Stripe の決済画面でご入力いただいたもの）</li>
                <li>・購入日時、およびご利用いただける期限（購入から1年）</li>
                <li>・ご入力いただいた計算の条件（退職金の見込額、勤続年数、確定拠出年金等の残高、加入期間、受け取り予定の年齢など。ご購入の前に無料版でご入力いただいた内容を含みます）</li>
                <li>・計算結果を開き直すための識別子（推測できない文字列）</li>
                <li>・決済に関する情報（決済代行事業者が発行する決済の識別子）。クレジットカード番号は取得・保存しません。</li>
              </ul>
              <p className="mt-3">
                次のものは取得・保存しません。
              </p>
              <ul className="mt-3 list-none space-y-1">
                <li>・クレジットカード情報（Stripe が取り扱い、当社は保持しません）</li>
                <li>・氏名、住所、電話番号</li>
                <li>・ご購入いただかなかった場合の、無料版でのご入力内容（無料版では保存しません）</li>
              </ul>
              <h3 className="mt-6 text-base font-semibold text-slate-900">
                15-2. ご家族の情報
              </h3>
              <p className="mt-3">
                有料版は、ご本人またはご家族のためにご利用いただけます。
                ご家族の情報をご入力になる場合は、そのご家族の了解を得たうえでご入力ください。
                ご入力いただいたご家族の情報は、ご本人の情報と同じ取り扱いをします。
              </p>
              <h3 className="mt-6 text-base font-semibold text-slate-900">
                15-3. 利用目的
              </h3>
              <ul className="mt-3 list-none space-y-1">
                <li>・計算結果の再表示および再計算</li>
                <li>・計算結果を開き直すリンクの送付</li>
                <li>・お問い合わせへの対応</li>
                <li>・返金その他、お支払いに関するお手続き</li>
              </ul>
              <h3 className="mt-6 text-base font-semibold text-slate-900">
                15-4. 保存期間
              </h3>
              <p className="mt-3">
                ご利用いただける期間（購入から1年）が経過したのち、60日以内に、メールアドレスとご入力の内容を削除します。
                ただし、法令により保存が必要な取引の記録については、この限りではありません。
              </p>
              <h3 className="mt-6 text-base font-semibold text-slate-900">
                15-5. 保管先・外部委託・外国にある第三者への提供
              </h3>
              <p className="mt-3">
                取得した情報は、Supabase（Supabase Inc. 提供）の東京リージョンのデータベースに、アクセス制御を施したうえで保管します。
                有料版で取得した情報を、法令に基づく場合を除き、ご本人の同意なく第三者に提供することはありません。
                機能の運営のため、次の外部事業者に業務を委託しています。一部は日本国外（米国）に所在します。
              </p>
              <ul className="mt-3 list-none space-y-1">
                <li>・Stripe, Inc.（決済処理・米国）。メールアドレスと決済に関する情報を取り扱います。</li>
                <li>・Resend（メールの送信・米国）。メールアドレスとメールの本文を取り扱います。</li>
                <li>・Supabase Inc.（データベース。東京リージョンで保管）</li>
                <li>・Vercel（ホスティング）</li>
              </ul>
              <p className="mt-3">
                当社は、各委託先に対し、本条の利用目的の範囲内で必要な情報のみを提供します。
                有料版をご購入いただくことで、これらの委託先への提供（外国にある第三者への提供を含む）に同意いただいたものとします。
                このことは、購入の前にご覧いただく画面にも表示しています。
              </p>
              <h3 className="mt-6 text-base font-semibold text-slate-900">
                15-6. 開示・訂正・削除等の請求
              </h3>
