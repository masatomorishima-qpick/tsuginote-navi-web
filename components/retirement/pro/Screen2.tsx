  ...
  return wakachi(
    <div ref={rootRef}>
      ...
      */}
      ...
      {/* 【2026-09-19・決め1362 ①（戦術Cowork `kaihatsu_ate_20260919q.md` C）】申告書の注記2か所を1つに。前の「（出さない場合は下の※をご覧ください）」と、
          下に在った `div.src`（20.42%の※）を外し、その中身をここに入れました。太字は「公的医療保険料・介護保険料は入っていません。」だけ。
          基準HTML（228,480 ／ 9b8875bc）696行から1字1句。 */}
      <p className="mt-3 text-[13px] leading-relaxed text-[#5b6470]">
        ※あなたが「退職所得の受給に関する申告書」を出す前提で計算しています。出さない場合は、あなたの退職金の収入金額の20.42%がいったん源泉徴収されます（所得税法201条3項＋復興特別所得税）。確定申告で精算され、最終的な税額は変わりません。変わるのは、いったん引かれる額と、戻ってくるまでの時間だけです。
        <br />
      ...
      */}
      <div className="mt-5 rounded-xl bg-slate-50 p-4 sm:p-5">
        <p className="text-base leading-relaxed text-slate-900">
          {/* 【2026-09-19・決め1362 ③（`kaihatsu_ate_20260919q.md` C）】見出しを短くし、4つの前提の行の太字を外し、下の2段落（決め1269 の1行を含む）を1段落に。
              4つの前提の行の字は変えていません。基準HTML（228,480 ／ 9b8875bc）710〜717行から1字1句。 */}
          <b className="font-bold">この画面の金額は、うかがっていないことを次のように置いて計算しています。</b>
        </p>
        <ul className="mt-2 list-disc space-y-1 pl-5 text-base leading-relaxed text-slate-900">
          ...
          <li>
            退職金を受け取るのは … 今年（{r.genzaiNen}年）（税金の決まりは年ごとに変わります）
          </li>
        </ul>
        <p className="mt-2 text-base leading-relaxed text-slate-900">
          あなたに当てはまるものがあると、実際の金額は多くなることも、少なくなることもあります。無料版は、受け取る年だけを計算します。これらと、退職した翌年以降の税と社会保険料は、有料版で計算します。
        </p>
      </div>
      ...
      */}
      ...
      {/* 【2026-09-19・決め1362 ①】ここに在った申告書の※（`div.src`・20.42%）は、上の注記（手取りの表の下）に1つにまとめて外しました。 */}
