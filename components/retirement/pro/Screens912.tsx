...
export function gyouNashi912(m: Moto912): string[] {
  ...
  /**
   * ★★★【2026-09-14・決め1180】**`tai_uchiwake_bun` は「行だけ落とす」ほうに移しました。**
   *
   * ★★【直す前は何が起きていたか】★`gamen11Bun.ts` は1本だけの方に `null` を返し、
   *   ★覚え書きには「行ごと落ちます ── 表の中ですので、その行だけを落とします」と書いてありました。
   *   ★★★**これは誤りでした。**★`gamenBun.ts` 324〜341行の `null` は
   *     `naWoHirou(b)`（★かたまり全体の名前）で見ていますので、
   *     ★★**表の中のセルが1つでも `null` なら、その表はかたまりごと落ちます。**
   *   ★★実測（★250人）…… ★**表がまるごと落ちる方 84人**／★そのうち**節が空になる方 68人**
   *     （★見出し「あなたの{tai_gen}（{tai_age}）」だけが、中身なしで残っていました）。
   *   ★★★**この覚え書きを書いたのは開発Coworkです。**★読んだつもりで、動かして確かめていませんでした。
   *
...
export const OCHITE_YOI: Readonly<Record<string, Readonly<Record<string, number>>>> = {
   ...
   *
   * ★★★【2026-09-25・決め1410（戦術Cowork `kaihatsu_ate_20260922m.md` 1-1・★数は戦術Coworkが基準HTMLを数えたもの）・森嶋さんの実装の承認は決め1437】
