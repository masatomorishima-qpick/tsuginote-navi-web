 ...
 *
 * もと: bin/senjutsu/tsuginote_gamen_base.html（180,674バイト ／ md5 f0699e820b8c28b6a8b79ad1a484ae11）
 */

/** 表の1行。`cells` は左から順のセル。`na` は、この行に出る `{名前}` の一覧 */
export type Gyou9shosai = {
...
export const GAMEN9shosai: readonly Block9shosai[] = [
  ...
  { kind: 'hon', bun: "{handan_a_bun}\n{handan_b_bun}\n{handan_c_bun}", na: ["handan_a_bun","handan_b_bun","handan_c_bun"] },
  { kind: 'midashi', lv: 3, bun: "保険料判定所得と保険料負担の比較" },
  { kind: 'hako', bun: "{an_a_bun}{an_b_bun}", na: ["an_a_bun","an_b_bun"] },
  { kind: 'hon', bun: "破線＝保険料・医療費の境目（濃い線が、あなたが実際に越える境目です）", na: [] },
  { kind: 'ret', koumoku: [
    { bun: "軽減が7割から5割に下がる {sakaime_1}", na: ["sakaime_1"] },
    { bun: "5割から2割 {sakaime_2}", na: ["sakaime_2"] },
    { bun: "軽減がなくなる {sakaime_3}", na: ["sakaime_3"] },
  ] },
  { kind: 'hon', bun: "あなたの公的年金は{koteki_kaishi_age}から始まります。{nenkin_gen}を受け取る期間が{koteki_kaishi_age}にかかると、その年は公的年金と{nenkin_gen}が同じ年の所得になり、あなたの所得が増えます。所得が国の定める基準を超えると、公的医療保険料・介護保険料・医療費の負担が上がることがあります。", na: ["koteki_kaishi_age","nenkin_gen"] },
  { kind: 'midashi', lv: 3, bun: "保険料判定所得の計算方法" },
  { kind: 'hon', bun: "{hantei_age}のあなたの所得を、計算の順番のまま並べました。", na: ["hantei_age"] },
  { kind: 'hyo', gyou: [
    { cells: ["{hantei_age}のあなた","{an_a_nensu}で\n受け取る","{an_b_nensu}で\n受け取る"],
      na: ["hantei_age","an_a_nensu","an_b_nensu"],
      kazari: [[null],[null,null],[null,null]] },
    { cells: ["公的年金\n{koteki_tsukisu}分","{a_koteki}","{b_koteki}"],
      na: ["koteki_tsukisu","a_koteki","b_koteki"],
      kazari: [[null,"tbls"],[null],[null]] },
    { cells: ["{nenkin_gen}","{a_ideco}","{b_ideco}"],
      na: ["nenkin_gen","a_ideco","b_ideco"],
    ...
    { cells: ["雑所得\n{zatsu_chu}","{a_zatsu}","{b_zatsu}"],
      ...
      kazari: [[null,"tbls"],[null],[null]] },
    { cells: ["あなたの給与所得","{a_kyuyo}","{b_kyuyo}"],
      na: ["a_kyuyo","b_kyuyo"],
      kazari: [[null],[null],[null]] },
    { cells: ["↓ ここから保険料の話です（税金の計算には出てきません）"],
      na: [],
      kazari: [[null]],
      gyoKazari: "shikiri" },
    { cells: ["65歳以上の15万円","{a_koujo15}","{b_koujo15}"],
      na: ["a_koujo15","b_koujo15"],
      kazari: [[null],[null],[null]] },
    { cells: ["保険料の判定に使う所得","{a_hantei_shotoku}","{b_hantei_shotoku}"],
      na: ["a_hantei_shotoku","b_hantei_shotoku"],
      kazari: [[null],[null],[null]],
      gyoKazari: "sum" },
  ] },
  ...
  { kind: 'hyo', gyou: [
    { cells: ["{hantei_age}のあなた","{an_a_nensu}で\n受け取る","{an_b_nensu}で\n受け取る"],
      na: ["hantei_age","an_a_nensu","an_b_nensu"],
      kazari: [[null],[null,null],[null,null]] },
    { cells: ["国民健康保険料などの軽減\n基準 {kokuho_kijun}","{a_kokuho_bun}","{b_kokuho_bun}"],
      na: ["kokuho_kijun","a_kokuho_bun","b_kokuho_bun"],
      kazari: [[null,"tbls"],[null],[null]] },
    { cells: ["住民税の非課税\n基準（{setai_kubun}） {hikazei_gendo}","{a_jumin_bun}","{b_jumin_bun}"],
      na: ["setai_kubun","hikazei_gendo","a_jumin_bun","b_jumin_bun"],
      kazari: [[null,"tbls"],[null],[null]] },
  ] },
