/**
 * lib/loan/articles.ts — 住宅ローン記事のレジストリ（2026-07-29 新設）
 *
 * ここが記事メタ情報の**唯一の正**である。
 *
 * 参照している場所：
 *   ・/loan ハブの記事一覧（app/loan/page.tsx）
 *   ・sitemap.xml（app/sitemap.ts）
 *   ・各記事ページの metadata / 構造化データ / パンくず / メインビジュアル
 *     （components/loan/LoanArticle.tsx 経由）
 *
 * 記事を1本増やす手順：
 *   1. 下の ARTICLES に1件足す（order で並び順を決める）
 *   2. app/loan/<path>/page.tsx に本文（目次・FAQ・セクション）だけを書く
 *   これでハブの一覧・sitemap・パンくず・Article/BreadcrumbList・OGP・
 *   メインビジュアルはすべて自動で揃う。個別に更新する場所は他にない。
 *
 * URL の方針（重要）：
 *   2026-07-29 に中カテゴリ /loan/karikae をページとして廃止したが、
 *   **記事のURLは変更していない**（インデックス済みの評価を失わないため）。
 *   パスに karikae が残っているのはその名残で、将来カテゴリを復活させる
 *   ときはそのまま使える。
 */

/** 記事のメインビジュアル（H1・最終更新日の下に表示し、Article の image にも使う） */
export interface MainVisual {
  /** サイトルートからのパス（例：/loan/karikae-demerit.webp） */
  src: string;
  /** 記事の内容を表す日本語。「アイキャッチ」等の無内容な語は使わない */
  alt: string;
}

export interface LoanArticle {
  /** サイトルートからのURL。レジストリのキーになる */
  path: string;
  /** meta title・OGP・一覧の見出しに使う */
  title: string;
  /** 記事ページの H1。title より説明的でよい */
  heading: string;
  /** パンくずの末尾に出す短い名前 */
  breadcrumb: string;
  /** meta description（検索結果用・120字前後） */
  description: string;
  /** /loan の一覧に出す説明（1〜2行）。description とは用途が違うので別に持つ */
  summary: string;
  /**
   * /llms.txt に出す「この記事が何を扱っているか」の一文。
   *
   * **具体的な金額・金利をここに書かないこと。** llms.txt に数値を持たせると、
   * 記事側の数値を更新したときに必ず取り残されて、AI検索へ古い数字を渡すことになる
   * （2026-07 に実際に発生した。手書きの llms.txt が修正前の費用のまま残っていた）。
   * 数値は記事本文（＝更新される場所）に任せ、ここは主題の案内に徹する。
   */
  topic: string;
  /** 'YYYY-MM-DD'。ISO 8601 への変換は LoanArticle.tsx 側で行う */
  datePublished: string;
  dateModified: string;
  visual: MainVisual;
  /** 一覧・sitemap の並び順（小さいほど先）。判断の流れに沿った論理順にする */
  order: number;
}

/**
 * 記事の一覧。
 * 並び順は「変動か固定か」→「費用はいくらか」→「いつ動くか」→「損をしないか」
 * という判断の流れに沿わせている。
 */
const ARTICLES: LoanArticle[] = [
  {
    path: '/loan/hendo-kotei',
    title: '住宅ローンは変動と固定どちらがいいか｜2026年7月の金利で試算',
    heading: '住宅ローンは変動と固定どちらがいいか｜2026年7月の金利で試算して比べる',
    breadcrumb: '変動と固定どちらがいいか',
    description:
      '2026年7月時点で変動は年1%前後、固定（フラット35）は3.14%。金利差は約2.1%です。いま固定に切り替えると月2〜4万円増え、変動が3.3%前後まで上がって初めて総支払額が並びます。残高・残り年数別の表で示します。',
    summary:
      'すでに変動で借りている人向けに、いま固定へ切り替えると月々いくら増えるのか、変動が何%まで上がったら切り替えた方が総支払額が少なくなるのかを、残高・残り年数ごとの表で示しています。',
    topic:
      '変動金利で借りている人が固定に切り替えるべきかを扱っています。切り替えたときに月々の返済がどう変わるか、変動が何%まで上がると総支払額が並ぶかを、残高・残り年数ごとの表で示しています。',
    datePublished: '2026-07-28',
    dateModified: '2026-07-28',
    visual: { src: '/loan/hendo-kotei.webp', alt: '変動金利と固定金利の分かれ道を表した図' },
    order: 1,
  },
  {
    path: '/loan/5nen-rule',
    title: '住宅ローンの5年ルール・125%ルールとは｜返済額が変わらない仕組み',
    heading: '住宅ローンの5年ルール・125%ルールとは｜返済額が変わらない仕組みと未払利息',
    breadcrumb: '5年ルール・125%ルール',
    description:
      '返済額が変わらないことと、負担が増えていないことは違います。金利が上がると元金の減り方が鈍り、やがて未払利息が発生します。発生する金利は「返済額×12÷残高」で自分で計算できます。',
    summary:
      '金利が上がっても返済額が5年間変わらない仕組みと、その間に元金の減り方がどう変わるかを解説。未払利息が発生する金利は自分で計算できます。',
    topic:
      '金利が上がっても返済額が一定期間変わらない仕組み（5年ルール・125%ルール）と、その間に元金の減り方がどう変わるかを扱っています。未払利息が発生する金利を自分で計算する方法も示しています。',
    datePublished: '2026-07-29',
    dateModified: '2026-07-29',
    visual: { src: '/loan/5nen-rule.webp', alt: '返済額が変わらないまま元金と利息の内訳が変わることを表した図' },
    order: 2,
  },
  {
    path: '/loan/karikae/hiyou',
    title: '住宅ローンの借り換え費用はいくら？手数料の内訳と元が取れる条件',
    heading: '住宅ローンの借り換え費用はいくら？手数料の内訳と、元が取れる条件',
    breadcrumb: '借り換え費用と元が取れる条件',
    description:
      '住宅ローンの借り換え費用は借入額の2.8〜3.4%程度、残高3,000万円なら約87万円です。事務手数料と登録免許税が借入額に比例して増えます。費用を引いた後にいくら残るのかを残高・残り年数・金利差ごとの表で示します。',
    summary:
      '費用は借入額の2.8〜3.4%程度、残高3,000万円なら約87万円。事務手数料と登録免許税が借入額に比例して増えます。費用を引いた後にいくら残るのかを、残高・残り年数・金利差ごとの表で示しています。',
    topic:
      '借り換えにかかる費用の内訳（事務手数料・登録免許税・司法書士報酬など）と、費用を払っても元が取れる条件を扱っています。',
    datePublished: '2026-07-28',
    dateModified: '2026-07-28',
    visual: { src: '/loan/karikae-hiyou.webp', alt: '借り換え費用の内訳を積み上げで表した図' },
    order: 3,
  },
  {
    path: '/loan/karikae/timing',
    title: '住宅ローンの借り換えはいつがベストなタイミングか',
    heading: '住宅ローンの借り換えはいつがベストなタイミングか｜判断できることとできないこと',
    breadcrumb: '借り換えのタイミング',
    description:
      '住宅ローンの借り換えのタイミングは、金利ではなく自分の条件で決まります。金利の底は誰にも予測できません。残りの返済期間が10年を切ると費用倒れになりやすく、1年待つだけでメリットは7万〜18万円減ります。',
    summary:
      '金利の底は誰にも予測できません。判断できるのは自分の条件です。残りの返済期間が10年を切ると費用倒れになりやすいこと、1年待つとメリットが7万〜18万円減ることを、残高・残り年数ごとの表で示しています。',
    topic:
      '借り換えをいつ実行するかを扱っています。金利の予測ではなく、残りの返済期間・年齢・団信・審査といった自分の条件から判断する考え方です。',
    datePublished: '2026-07-29',
    dateModified: '2026-07-29',
    visual: { src: '/loan/karikae-timing.webp', alt: '時間の経過とともに借り換えメリットが減ることを表した図' },
    order: 4,
  },
  {
    path: '/loan/karikae/demerit',
    title: '住宅ローン借り換えのデメリット｜損をする6つのケース',
    heading: '住宅ローン借り換えのデメリット｜損をする6つのケースと確認すべきこと',
    breadcrumb: '借り換えのデメリット',
    description:
      '住宅ローンの借り換えで損をするのは主に6つのケースです。費用を回収できない、住宅ローン控除が受けられなくなる（約70万円の損）、月々の返済が下がっても総額では87万円増える、など数字で確認します。',
    summary:
      '費用を回収できない、住宅ローン控除が受けられなくなる、月々が下がっても総額は増える——借り換えで損をするケースを数字で確認します。控除の喪失は残高2,000万円・残り5年で約70万円になります。',
    topic:
      '借り換えで損をするケースを扱っています。費用倒れ、住宅ローン控除が受けられなくなること、返済期間を延ばすと総額が増えることなどです。',
    datePublished: '2026-07-29',
    dateModified: '2026-07-29',
    visual: { src: '/loan/karikae-demerit.webp', alt: '見えにくい部分に費用が隠れていることを表した図' },
    order: 5,
  },
  {
    /* 2026-07-31 追加：記事6。
     * order を末尾（6）にしたのは、この記事自体が「まず借り換えで下げられる金利差を
     * 確認し、それが取れない場合に繰り上げ返済へ」という順序を主張しているため。
     * 借り換え3本（order 3〜5）より前に置くと、ハブの並びが記事の主張と逆になる。 */
    path: '/loan/kuriage-hensai',
    title: '住宅ローンの繰り上げ返済は得か｜期間短縮型と返済額軽減型の違い',
    heading: '住宅ローンの繰り上げ返済は得か｜期間短縮型と返済額軽減型の違いと、やらない方がいい人',
    breadcrumb: '繰り上げ返済',
    description:
      '住宅ローンの繰り上げ返済を、期間短縮型と返済額軽減型の違い、住宅ローン控除との関係、借り換えとの比較から整理します。やらない方がいい人も数字で示します。',
    summary:
      '期間短縮型と返済額軽減型は何が違うのか。控除期間中に繰り上げ返済していいのか。借り換えとどちらを先にすべきかを、自分の数字で判断するための記事です。',
    topic:
      '繰り上げ返済の方法の違い、住宅ローン控除との関係、借り換えとの優先順位、変動金利のリスクとの関係',
    datePublished: '2026-07-31',
    /* 2026-07-31：償還期間10年要件の判定基準を国税庁の質疑応答事例にもとづいて
     * 書き換えたため更新（公開日と同日のため値は変わらない）。 */
    dateModified: '2026-07-31',
    visual: {
      src: '/loan/kuriage-hensai.webp',
      alt: '期間短縮型と返済額軽減型で減るものが違うことを表した図',
    },
    order: 6,
  },
  {
    /* 2026-07-31 追加：記事7。
     * 記事6が「控除期間中に繰り上げていいのか」を扱うのに対し、こちらは
     * 控除が終わった人・最終年の人に限定した続編なので、記事6の次（order 7）に置く。 */
    path: '/loan/kojo-shuryo-kuriage',
    title: '住宅ローン控除が終わったら繰り上げ返済すべきか｜実行する月まで計算',
    heading: '住宅ローン控除が終わったら繰り上げ返済すべきか｜実行する月まで計算で決める',
    breadcrumb: '控除終了後の繰り上げ返済',
    description:
      '住宅ローン控除が終わった後の繰り上げ返済を計算で整理します。最終年の人が実行すべき月、金額別の効果、一括返済との比較、借り換えを先に確認すべき条件まで扱います。',
    summary:
      '控除が終わると繰り上げ返済のブレーキが外れます。最終年なら12月ではなく翌1月。一括返済の代償と、先に借り換えを計算すべき条件も数字で示します。',
    topic:
      '控除終了後の繰り上げ返済のタイミング、最終年の実行月、一括返済との比較、借り換えとの優先順位',
    datePublished: '2026-07-31',
    dateModified: '2026-07-31',
    visual: {
      src: '/loan/kojo-shuryo-kuriage.webp',
      alt: '住宅ローン控除の期間が終わった後に繰り上げ返済の効果が現れることを表した図',
    },
    order: 7,
  },
  {
    /* 2026-08-01 追加：記事8。
     * v2.1（Excel出力）の入口として「エクセル 作り方」で検索した人を迎える記事。
     * ツールは既定の借り換え・金利モードのまま（この記事の主題は償還表で、
     * 繰り上げ返済は一節にすぎないため defaultMode を指定しない）。 */
    path: '/loan/hensai-yoteihyo-excel',
    title: '住宅ローンの返済予定表をエクセルで作る方法｜関数の意味から繰り上げ返済まで',
    heading: '住宅ローンの返済予定表をエクセルで作る方法｜関数の意味から繰り上げ返済の反映まで',
    breadcrumb: '返済予定表をエクセルで作る',
    description:
      '住宅ローンの返済予定表をエクセルで作る手順を、PMT関数の引数の意味から説明します。繰り上げ返済の反映、間違えやすい点、検算のしかたまで扱います。',
    summary:
      '必要な列は5つ、式は4つだけです。PMT関数がなぜその書き方になるのかから説明します。繰り上げ返済の組み込み方と、気づけない間違いも扱います。',
    topic:
      '返済予定表の作り方、PMT関数の引数の意味、元金均等返済との違い、繰り上げ返済の表への反映、未払利息のラインの列',
    datePublished: '2026-08-01',
    dateModified: '2026-08-01',
    visual: {
      src: '/loan/hensai-yoteihyo-excel.webp',
      alt: '返済予定表の各列が前の行の残高から順に計算されていくことを表した図',
    },
    order: 8,
  },
];

/** 表示順に並べた記事一覧（ハブと sitemap が使う） */
export const LOAN_ARTICLES: LoanArticle[] = [...ARTICLES].sort((a, b) => a.order - b.order);

/**
 * パスから記事を取得する。各記事ページの先頭で呼ぶ。
 * 登録がなければビルド時に落とす（レジストリへの追加漏れをその場で気づけるようにする）。
 */
export function getLoanArticle(path: string): LoanArticle {
  const found = ARTICLES.find((a) => a.path === path);
  if (!found) {
    throw new Error(
      `[lib/loan/articles] ${path} がレジストリに登録されていません。lib/loan/articles.ts の ARTICLES に追加してください。`,
    );
  }
  return found;
}
