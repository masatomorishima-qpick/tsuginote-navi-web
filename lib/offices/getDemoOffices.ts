...
function mapOfficeRow(row: ExistingOfficeRow): Office {
  return {
    ...
    // デモ用は null で全落ちしにくいよう、主要条件は true 寄りで扱う
    supportsTokyo: row.supports_tokyo ?? true,
    handlesInheritanceRenunciation:
      row.handles_inheritance_renunciation ?? true,
    ...
    acceptingNewCases: row.accepting_new_cases ?? row.is_accepting ?? true,
...
export async function getDemoOffices(): Promise<Office[]> {
  ...
  if (error) {
    console.error("[getDemoOffices] failed", {
    ...
    throw new Error(
      `デモ用事務所データの取得に失敗しました: ${error.message}${
