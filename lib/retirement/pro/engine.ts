...
export function taishokuShotokuKubun(gs: [Gen, number][],
  ...
  /**
   * ★★★【2026-09-14・決め1190】**一般の区分が無いときは、残りを短期に付けます。**
   *
   * ★★【直す前は何をしていたか】★残りをいつも一般（`iKojo`）に置いていました。
   *   ★★一般の区分が無い方では、★**その控除がどの区分にも使われず、消えていました**
   *     （★実測 `seed 183`・400,000円・税が 203,360円 多い）。
   *
   * ★★【法令の姿】★国税庁「令和7年版 源泉徴収のあらまし」第3・**126ページ**
   *   （★短期退職手当等及び特定役員退職手当等の両方の支給がある場合）──
   *   ★★**「短期退職所得控除額 ＝ 退職所得控除額 − 特定役員退職所得控除額」**
   *   ★★★ですので ★**「区分が無いのに割り振られた余り」というものは、法令の側には在りません。**
   *
   * ★★4つの組み合わせを並べると、決まりは1つです（★戦術Cowork 1-1）──
   *   ★**特定役員の控除は式で出す。残りの区分のうち、いちばん外側の1つが
   *     「退職所得控除額 − ほかの区分の控除」を取る。**
   *     ★一般が在る …… 一般が取る（★一般＋特定役員／一般＋短期／3区分）
   *     ★★一般が無い …… **短期が取る**（★短期＋特定役員・126ページ）
   *
   * ★★★**区分が1つだけの年は、下の E-18 の枝がそのまま `kojoAdj` を置きます**ので、
   *   ★ここは**区分が2つ以上の年だけ**に効きます。
   */
  if (!ippan.length && tanki.length) {
    sKojo = kojoAdj - tKojo;
    iKojo = 0;
  }
  ...
  if (kubunKazu === 2) {
    ...
    /**
     * ★★**3区分**（★(D)・決め1174・★決め1191）。
     *   ★マイナスの額を2分の1し、残る2つからそれぞれ差し引きます。
     *   ★引ききれない分は、もう一方から差し引きます。
     *
     * ★★★【2026-09-14・こちらの誤りを直しました】**前の `wakeru()` の式は誤りでした。**
     *   ★前 …… `[x - H - yAmari, y - H - xAmari]`（`xAmari = max(0, H - max(0,x))`）
     *   ★★これだと ★**引ききれない区分から、引ききれない分をもう一度引いて**いました。
     *     ★例：x が 0・y が 1,000,000・H が 100,000 のとき ──
     *       ★前 …… x が **−100,000**／y が 800,000（★合わせて 300,000 引いた＝**移す額 200,000 を超えます**）
     *       ★いま … x が **0**／y が 800,000（★合わせて 200,000＝**移す額と同じ**）
     *   ★★★**250人では1度も通っていません**（★3区分の方は1人で、マイナスが2つあり門で止まっていました）。
     *     ★ですので、★**誤った数をお出ししたことはありません。**
     */
    const kubunNa3 = ['特定役員', '短期', '一般'] as const;
    /** ★★（注）を1つ当てます …… `k` の区分のマイナスを、残る2つに**半分ずつ**移します */
    const ateru = (z: readonly [number, number, number], k: number): [number, number, number] => {
      const A = -z[k];
      /**
       * ★★★**門2** …… ★2分の1に1円未満の端数が出る形（★マイナスが奇数）。
       *   ★（注）は「どちらへ切り上げ、どちらへ切り捨てるか」を書き分けていますが、
       *   ★★**一般がマイナスのときの向きを、こちらでは確かめられていません。**★**止めます。**
       */
      if (A % 2 !== 0) {
        throw new Error(`調整計算で移す額（${A}円・${kubunNa3[k]}）が奇数ですので、`
          + '2分の1に1円未満の端数が出ます。'
          + '**どちらへ切り上げ、どちらへ切り捨てるかを、こちらでは確かめられていません。**'
          + 'こちらでは決めません。戦術Coworkに投げてください。'
          + `［特定役員 残り${z[0]}　短期 残り${z[1]}　一般 残り${z[2]}　その年の控除 ${kojoAdj}］`);
      }
      const H = fdiv(A, 2);
      const [x, y] = [0, 1, 2].filter((j) => j !== k);
      const out: [number, number, number] = [z[0], z[1], z[2]];
      out[k] = 0;
      /** ★その区分が持っている分だけ引き、★引ききれない分は**もう一方**から引きます */
      const hikuX = Math.min(H, Math.max(0, out[x]));
      const hikuY = Math.min(H, Math.max(0, out[y]));
      out[x] = out[x] - hikuX - (H - hikuY);
      out[y] = out[y] - hikuY - (H - hikuX);
      return out;
    };
    /** ★マイナスの区分を、渡された順に1つずつ当てます */
    const junni = (jun: readonly number[]): [number, number, number] => {
      let z: [number, number, number] = [tZan, sZan, iZan];
      for (const k of jun) if (z[k] < 0) z = ateru(z, k);
      return z;
    };
    const mainasu = [0, 1, 2].filter((k) => [tZan, sZan, iZan][k] < 0);
    if (mainasu.length === 1) {
      [tZan, sZan, iZan] = junni(mainasu);
    } else if (mainasu.length >= 2) {
      /**
       * ★★★【2026-09-14・決め1191】**2つ以上がマイナスのとき。**
       *
       * ★（注）1〜3は**1つずつ**書かれていて、★★**2つが同時のときの順は書かれていません。**
       * ★★★ですので ── ★**順を変えて2通り出し、1円まで同じかを数えます。**
       *   ★**同じなら、順を決める要りがありません**（★どちらの順でもよい）。
       *   ★★**違うなら、そこで止めます**（★そのときは戦術Coworkが条文をもう一度さがします）。
       */
      const a = junni(mainasu);
      const b = junni([...mainasu].reverse());
      if (a[0] !== b[0] || a[1] !== b[1] || a[2] !== b[2]) {
        throw new Error('その年の3つの区分のうち、2つ以上で収入が控除に満たしていません。'
          + '**（注）を当てる順で答えが変わります。**こちらでは決めません。戦術Coworkに投げてください。'
          + `［${mainasu.map((k) => kubunNa3[k]).join('→')} の順 …… `
          + `特定役員${a[0]}／短期${a[1]}／一般${a[2]}`
          + `　／　逆の順 …… 特定役員${b[0]}／短期${b[1]}／一般${b[2]}`
          + `　／　その年の控除 ${kojoAdj}］`);
      }
      [tZan, sZan, iZan] = a;
    }
  ...
  /**
   * ★★★【2026-09-14・決め1184】**門3（300万円超で止める）は、外しました。**
   *
   * ★★【前の回に何を止めていたか】★一般＋短期で調整計算が働き、短期の残りが300万円を超える方を
   *   止めていました。★実測 **21人／250**。
   * ★★★【なぜ外したか】★戦術Coworkが（注）を読み直されました ──
   *   ★「確かめられなかった」とされた **⑴ロ（注）2 は「①がマイナスの場合」の話**ですが、
   *   ★★**ロの枝に入るのは、①が300万円を超えたときだけ**です。
   *   ★★★**ロで①がマイナスになることは、ありえません。**★ですので21人には当たりません。
   *   ★21人に当たるのは **⑴ロ（注）3**（★一般がマイナス → 短期の①**及び**③から差し引く）で、
   *     ★**イの（注）3と1字1句おなじ**です（★戦術Coworkが2つを並べて確かめられました）。
   *
   * ★★★**代わりに、起きないはずの所へ門を1つ置きます**（★決め1140の形・戦術Cowork 1-2）──
   *   ★**①（調整する前の短期の残り）がマイナスなのに、③の枝（300万円超）に入ったら止める。**
   *   ★★短期がマイナスの側のときは、その残りは調整で0になりますので、★**ここは通らないはず**です。
   */
  const sZanMoto = sShunyu - sKojo;
  if (sZanMoto < 0 && sZan > Z.TANKI_KIJUN) {
    throw new Error(`調整する前の短期の残りが ${sZanMoto}円（マイナス）なのに、`
      + `調整のあとの残り ${sZan}円 が300万円を超えました。`
      + '**この形は起きないはずです。**こちらでは決めません。戦術Coworkに投げてください。');
  }
   ...
   *   ★★決め1184で門3を外しましたので、★**300万円を超える方も、ここを通ります**（★実測 21人）。
   */
...
/** 一時金を受け取る1つの年ぶんの、確定申告の答え */
export interface ShinkokuGyou {
  /** ★判定した所得の年（★「一時金を受け取る年」。★**その翌年ではありません**） */
  nen: number;
  /** ★その年に受け取る支給源の名前（★字に「何の一時金の年か」を書くため・決め917） */
  gens: string[];
...
/**
 * ★★★その受け取り方について、**一時金を受け取る年ごと**の確定申告の答え。
 *
 * ★戦術Cowork `senjutsu_20260908b.md` 決め916・917（2026-09-08）。
 * ★★`v5/engine.py` の `shinkoku_ichiran()` と**同じもの**です
 *   （★金の当てで、代表案12,500の**年ごと・支給源・理由まで**突き合わせています）。
 *
 * ──────────────────────────────────────────────────────────
 * ★★★【どの年を見るか】**「一時金を受け取る年すべて」です。「その翌年」ではありません。**
 *
 *   ★画面12の節の見出しは「あなたが退職した翌年」ですが、これは**手続きをする時期**です。
 *   ★確定申告は「**その年の所得について、翌年に申告する**」ものですから、
 *     **判定する所得の年は、一時金を受け取る年**になります。
 *   ★★★`shinkokuIru(p, year, …)` に**翌年を渡すと誤ります。**
 *
 * ★★★【`tai_shotoku > 0` で絞ってはいけません・決め916】
 *   ★2026-09-08 まで、この本は **`detail` の中で退職所得金額が0より大きい年**だけを見ていました。
 *   ★★★**「0」を「無い」と読んでいた誤りです**（★戦術Cowork `senjutsu_20260908b.md` 0節）。
 *     ★退職所得控除で退職所得金額が0になった方にも、**一時金を受け取る年はあります。**
 *   ★実測（`golden_light_20260906`・代表案10,000）……
 *     ・`keika` の行の合計 …… **14,182行**（★`tai_shotoku > 0` だけだと 11,146行）
 *     ・★★**年はあるのに、どの年も退職所得金額が0の案 …… 1,041案（10.4%）**
 *       ── ★前の形では、この方の節がまるごと消えていました。
 * ──────────────────────────────────────────────────────────
 *
 * ★★【空の並びを返すとき】`r.keika` が空のとき ＝ **一時金を受け取る年が1つも無い**とき。
 *   ★★★このときだけ、**画面12のこの節そのものを出しません**（★決め916で直した③）。
 *   ★実測 …… **6案（0.1%）**だけです。
 *
 * ★★【年が2つになることがあります】実測 …… **4,188案（41.9%）／いちばん多い案でも2つ**。
 *   ★★★そのうち**2つの年で答えが違う案が 2,819案（67.3%）**あります
 *     （★2つの年の離れ方は **1年あき〜20年あき**）。★1つにまとめられません（★決め917）。
 *   ★★2年目に受け取るものは、**91.9% が iDeCo か小規模企業共済**です（★退職金ではありません）。
 *     ── ★だから `gens` を運びます。
 */
export function shinkokuIchiran(p: Jinbutsu, r: EvalResult): ShinkokuGyou[] {
  const out: ShinkokuGyou[] = [];
  // ★★★`r.keika` は「一時金を受け取る年」1つにつき1行です（`taishokuByYear()`）。
  //   ★★こちらで回して測りました（代表案10,000）……
  //     ・年の小さい順でない所 …… **0か所**
  //     ・`keika` の年が `detail` に無い所 …… **0か所**
  //   ★ですので、並べ替えずに、そのまま回します。
  for (const k of r.keika) {
    const d = r.detail[k.year];
    const s = shinkokuIru(p, k.year, d.ideco_nenkin, d.tai_shotoku);
    out.push({ nen: k.year, gens: [...k.gens], ataru: !s.iru, riyu: shinkokuRiyu(s) });
  }
  return out;
}
 ...
 *   ★★同じ**向き**の所は**5か所**あります。★★★**うちこの本が当たるのは3か所**です
 *     （★2026-09-15・戦術Cowork `senjutsu_20260915b.md` 2節・決め1219 ── ★5か所をぜんぶ開いて数えました）。
 *
 *     ★★**当たる3か所**（★`plan`＝**案**を見ています）
 *       ・`gamen8.ts` の `zenToori()` の中 …… ★**この回に寄せました**
 *       ・`gamen9shosaiBun.ts` …… ★画面9詳細＝**v1.1**（★`tome.md` G ── いまは触りません）
 *       ・`gamen11Bun.ts` …… ★画面11＝**v1.1**（★同上）
 *
 *     ★★★**当たらない2か所**（★**年齢の数**を見ています。★案はまだ作られていません）
 *       ・`engine.ts` の `build()` の `for (const nAge of nenkinAges)` の中
 *       ・`chart10.ts` の `kotekiCash(p, kaishiAge: number, …)` …… ★引数が数です
 *
 *   ★★★決め1219 ＝ **「同じ行が何か所にある」と数える前に、その行が「同じものを見ているか」を数える。**
 *     ★こちらは便14lで「5か所」と**形だけで数えて**書きました。★戦術Coworkが開いて直してくださいました。
 *   ★★★**写しがあれば、次を書く人は必ず忘れます。**★ここを正本にします。
 *
