...
export function taishokuShotokuKubun(gs: [Gen, number][],
  ...
  if (kubunKazu === 2) {
    ...
    if (mainasu.length >= 2) {
      /**
       * ★★★【2026-09-14・戦術Cowork 便f 2-2】**門が鳴るときは、数字もいっしょに出します。**
       *   ★★止めるだけだと、★**そちらが条文を確かめるときの材料が残りません。**
       */
      throw new Error('その年の3つの区分のうち、2つ以上で収入が控除に満たしていません。'
        + '**この形は、国税庁の（注）が答えていません。**こちらでは決めません。戦術Coworkに投げてください。'
        + `［特定役員 収入${tShunyu}／控除${tKojo}／残り${tZan}`
        + `　短期 収入${sShunyu}／控除${sKojo}／残り${sZan}`
        + `　一般 収入${iShunyu}／控除${iKojo}／残り${iZan}`
        + `　その年の控除 ${kojoAdj}］`);
    }
    if (mainasu.length === 1) {
      const A = -mainasu[0];
      if (A % 2 !== 0) {
        throw new Error(`調整計算で移す額（${A}円）が奇数ですので、2分の1に1円未満の端数が出ます。`
          ...
          + `［特定役員 残り${tZan}　短期 残り${sZan}　一般 残り${iZan}　その年の控除 ${kojoAdj}］`);
      }
      const han2 = fdiv(A, 2);
