 ...
 */
/**
 * ★★【2026-09-22・戦術Cowork `kaihatsu_ate_20260922e.md` 4節】**列名を長い方に替えました。**
 *   ★「受け取り終わる年齢」→「退職金・iDeCo等を受け取り終わる年齢」。
 *   ★★**公的年金の受け取り終わりではない**ことを、列名そのものに入れるためです（★戦術Coworkの決め）。
 *   ★基準HTML（231,629 ／ `4a667a52…`）1036・1047・1440・1451・1506・1561・1572・1583行から1字1句。
 *   ★★列の幅は `retsuNoHaba()` が字から計算しますので、ここを替えると幅も自動で広がります
 *     （★`kensa/excel_haba_mon.tsx` が「###」と切れを数えます）。
 */
const RETSU_S2 = ['まとめの印', '番号', 'あなたの受け取り方', 'この受け取り方で増える税金（円）', '確定申告で戻る額（円）',
  'あなたの手取り（円）', '最初の年に入る額（円）', '退職金・iDeCo等を受け取り終わる年齢', '保険料・医療費'] as const;
