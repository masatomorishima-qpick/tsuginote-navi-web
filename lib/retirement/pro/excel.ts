...
export const KARA_NO_ATSUKAI: Record<string, string> = {
  ...
  /**
   * ★★★【2026-09-14・決め1211】**かっこの中には「エンジンが何として計算したか」だけを書きます。**
   *   ★★**「なぜ空なのか」を書きません**（★空の理由は、その方しか知りません）。
   *   ★前は「配偶者がいらっしゃらないため、この欄は使っていません」と書いていました。
   *     ★★戦術Coworkの止め …… ★**「配偶者がいらっしゃらないため」は理由の推し量りです**。
   *   ★★こちらの記録（★便に書きました）…… ★この欄が空のまま Excel まで来られるのは、
   *     ★`㉕/nai`（「あなたに配偶者はいない」）にお答えになった方だけです
   *     （`paidRules.ts` 294行 `kakusu` ／ 591〜594行 ／ 409〜413行 `kara: 'hissu'`）。
   *   ★★★**それでも、字は決め1211のとおりにします。**★字を決めるのは戦術Coworkです。
   */
  '㉑': 'この欄は使っていません',                        // paidRules.ts 591〜594行（★読みません）
  ...
  '㉗': '0人・「いいえ」として計算しました',                // paidRules.ts 601〜605行
};
 ...
 */
const RETSU_S1 = ['あなたの受け取り方', 'この受け取り方で増える税金（円）', 'あなたの手取り（円）',
  '保険料・医療費', '見方'] as const;
