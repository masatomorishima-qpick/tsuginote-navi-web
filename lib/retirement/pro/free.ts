...
export function freeResult(args: {
  ...
  const R = E.build(p, ['退職金'], 'iDeCo等', genzaiNen, {
    ...
    genzaiNen,
    /**
     * ⑱【回1・1-1・2026-09-04】★**`nenkinKaisu` を明示で渡します**（★既定を作りません）。
     *
     *   `engine.ts` の `build()` が `nenkinKaisu` を**必須**にしましたので、渡さないと止まります。
     *   ★★**無料版は⑱を聞いていません。**画面2に出すのは「年1回で受け取ったとき」の数です。
     *   ★★**これは既定値ではありません。**「無料版は⑱を聞かない」という決めを、ここに書いています。
     *   ★上の `nenkinAges: [p.koteki_kaishi_age]` と、同じ書き方にそろえています。
     *   ★★画面2に出している数は、この 1 で計算した数です。★⑱を聞くようにするなら、ここも変わります。
     */
    nenkinKaisu: 1,
  });
