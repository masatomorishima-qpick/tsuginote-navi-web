...
export interface Bun10 {
   ...
   */
  /**
   * ★★★【2026-09-14・決め1168】**`null` になるのは②が無い方だけ**です（★①の有無では変わりません）。
   */
  an_b_mijikai: string | null;
  /**
   * ★★★`{kurabe_bun}` …… 995行の1文（★2026-09-14・決め1172）。
   *
   * *   ★前は**印が1つも無い固定の字**で、「性質の違う2つを、それぞれ分けてお見せします。」と
   *     ★★**①と②の両方が在る75人にしか当たっていませんでした**（★合わない方 175人・70.0%）。
   * *   ★★①も②も無い方は `null` ＝ かたまりごと落ちます。
   */
  kurabe_bun: string | null;
  /**
   * ★★★`{gurafu_ji}` …… 「◯の数字に入っているもの」の節の見出しと囲み（★決め1172）。
   *
   * *   ★★見出しにも印を入れましたので、★**①も②も無い方（17人）は節ごと落ちます**
   *     （★決め1101・決め1118(5)と同じ形）。
   * *   ★これで「たては『その年齢まで受け取った場合』の数字です」も、★**囲みごと落ちます**。
   */
  gurafu_ji: string | null;
  /**
...
/**
 * ★★★`{kurabe_bun}` の字（★2026-09-14・決め1172・**基準HTMLの字をそのまま**）。
 *   ★`ryoho` …… ①と②の両方が在る方（★見本の字そのまま）
 *   ★`katahou` …… ①だけ・②だけの方（★「性質の違う2つを、それぞれ分けて」を外した字）
 */
const KURABE = {
  ryoho: 'あなたの年齢ごとに、手元にいくら入るかを比べます。性質の違う2つを、それぞれ分けてお見せします。',
  katahou: 'あなたの年齢ごとに、手元にいくら入るかを比べます。',
} as const;

/** ★★`{gurafu_ji}` の字（★決め1172） */
const GURAFU = { ryoho: '①と②のグラフ', an1: '①のグラフ', an2: '②のグラフ' } as const;
...
export function gamen10Bun(
  ...
  return {
    ...
    /**
     * ★★★【2026-09-14・決め1168】**②の2案から作ります**（★①の相手からは作りません）。
     *
     * *   ★1038行は「受け取り方は**どちらも**`{nenkin_gen}`を`{an_b_mijikai}`にそろえて」で、
     *     ★★**②で比べる2案は、`{nenkin_gen}` の受け取り方がどちらも同じ**です。★①とは関わりません。
     * *   ★★前は `kijunAri`（★①の相手が在るか）で `null` にしていましたので、
     *     ★★★**①が無く②が在る7人で、②の本文だけが落ちていました**（★開発Coworkが250人で数えました）。
     * *   ★★★**`null` になるのは②が無い168人だけ**です（★①の有無では変わりません）。
     * *   ★字そのものは変わりません ── ★どちらの案も `{nenkin_gen}` の受け取り方は
     *     ★**その方がいま選んでいる受け取り方**（`plan`）ですので、`anBMijikaiJi(plan, …)` のままです。
     */
    an_b_mijikai: aite === null ? null : anBMijikaiJi(plan, idecoName, p),
    sa_hajime_age: kijunAri ? `${ages[0]}歳` : null,
    // ★★決め1118(3) …… 相手が在る方には出しません（★かたまりごと落ちます）
    an_onaji_bun: kijunAri ? null : AN_ONAJI,
    /**
     * ★★★決め1172 …… **①と②の在る・無いで4通り**（★実測 75／151／7／17 ＝ 250）。
     *   ★★**こちらで数を決めていません** ── ★`kijunAri`（①）と `aite !== null`（②）だけを見ています。
     */
    kurabe_bun: !kijunAri && aite === null ? null
      : kijunAri && aite !== null ? KURABE.ryoho : KURABE.katahou,
    gurafu_ji: !kijunAri && aite === null ? null
      : kijunAri && aite !== null ? GURAFU.ryoho
      : kijunAri ? GURAFU.an1 : GURAFU.an2,
    /**
