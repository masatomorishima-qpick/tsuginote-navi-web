...
function kubunBunJi(k: E.KeikaRow, na: string): string | null {
  ...
  if (hanbun === 0) {
    /**
     * ★★★**「2分の1にしない部分」が0円の方**（★実測 **2人／250**・seed 21・seed 235）。
     *   ★★食い違いの**ぜんぶ**が「引けなかった控除」から出ています。
     *   ★★★**`kubunNa()` が名前を返す方も、ここに入ります**（★seed 21）──
     *     ★その区分は在りますが、★**その区分の収入が控除以下で、はみ出した額が0円**ですので、
     *     ★1つめの字（「その部分は2分の1にしませんので」）は**当たりません**。
     */
    if (amari === 0) {
      throw new Error(
        `食い違いが ${en(sa)} あるのに、2分の1にしない部分も引けなかった控除も0です。`
        + '`kubunUchiwake()` の分け方を確かめてください（決め1165）。',
      );
    }
    return `あなたの${na}は、その年に受け取る区分が2つ以上あります。`
      + `退職所得控除は区分ごとに分けて引きますので、ある区分で引き切れなかった控除は、`
      + `ほかの区分では引けません。そのため、あなたの退職所得は`
