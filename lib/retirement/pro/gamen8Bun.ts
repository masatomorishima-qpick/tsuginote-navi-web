...
export function gamen8Bun(
  ...
  return {
    ...
    sudeni: g.koteki_sudeni
      ...
      : '',
    // 【2026-09-24・戦術Cowork `kaihatsu_ate_20260924b.md` 2-1】「いったん源泉徴収されます」→「先に引かれます」・「いったん引かれる額」→「先に引かれる額」（基準HTML 231,055 ／ 745aefae の画面8の4か所と同じ字）
    shinkokusho:
      `「退職所得の受給に関する申告書」は、あなたが${taiName}を受け取るまでに勤め先へ出してください。`
      + `出さないと、あなたの${taiName}の収入金額の20.42%が先に引かれます`
      + '（所得税法201条3項＋復興特別所得税）。最終的な税額は変わりません。確定申告で精算されます。'
      + '変わるのは、先に引かれる額と、戻ってくるまでの時間だけです。'
      + 'このツールは出した場合で計算しています。',
    /**
    ...
    fileNakami: [
      ...
      ['2　受け取り方の一覧',
       // 【2026-09-22・戦術Cowork `kaihatsu_ate_20260922e.md` 3節・4節】
       //   「手取りの多い順に並んでいます。」と「まとめの印」を足し、列名を長い方に替えました。
       //   基準HTML（231,629 ／ 4a667a52）1063・1470・1525・1604行から1字1句（4か所とも同じ字です）。
       `${g.toorisu.toLocaleString('en-US')}通りすべて。手取りの多い順に並んでいます。まとめの印／番号／あなたの受け取り方／`
       + 'この受け取り方で増える税金（円）／確定申告で戻る額（円）／あなたの手取り（円）／'
       + '最初の年に入る額（円）／退職金・iDeCo等を受け取り終わる年齢／保険料・医療費。'
