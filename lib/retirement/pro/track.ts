 ...
 *
 * `/retirement/pro`（老後のお金の受け取りシミュレーション）のGA4計測。
...
/** §8-2 無料版（#1〜#18。実装指示書 v4 1050〜1067行が #1〜#17 で、#18 は 2026-09-18 に足した `pro_lp_scroll`） */
export type FreeEvent =
  | 'pro_lp_view'
  /**
   * 【§8-2 #18・2026-09-17 に足しました（戦術Cowork「まとめ便」お願い5）】画面1の読み進み。
   *
   * 【番号】#18。2026-09-18 に戦術Coworkが決めました
   *   （`kaihatsu_ate_20260918b.md` 3節）。
   *   数え方 …… 実装指示書 v4（136,156 ／ `e8b21f8596aa734f4a1ecc3acc7ce614` ／ 1,598行）の
   *   1050〜1067行が §8-2「無料版のイベント」で 17本。末尾に足したので、
   *   **既存の17本の番号は1つも動きません。**
   *
   * 【名前】画面2は `pro_result_scroll`、画面5-6は `pro_pricing_scroll` で、
   *   どちらも「その画面の呼び名＋`_scroll`」です。画面1の呼び名は `pro_lp_view` の `lp` なので、
   *   そろえました（新しい呼び名を作っていません）。
   */
  | 'pro_lp_scroll'
  | 'pro_input_start'
  | 'pro_input_field'
...
/** 画面5-6のブロック（§8-5。`nayami` は測らない ── ★2026-09-22 に `photo` から名前が替わりました・決め1401） */
/**
...
type Params = {
  ...
  //   画面1の「**あなたが入力した金額を、当社は保存しません。**」に、実装を合わせるためです。
  //   ★★【2026-09-16・決め1297】★字が替わりました（★前は「入力された内容は保存されません」）。
  //     ★★★**この1行を替えるとき、戦術Coworkは「Googleにも残りません」とは書きませんでした**
  //       ── ★**帯（範囲）は、いまも送っているためです**（★下の `taishoku_band`・`ideco_band`）。
 ...
 *   画面1の「**あなたが入力した金額を、当社は保存しません。**」には抵触しません
 *   （★★【2026-09-16・決め1297】★字が替わりました。★前は「入力された内容は保存されません」）。
 *   `gclid` は入力内容ではなく、
