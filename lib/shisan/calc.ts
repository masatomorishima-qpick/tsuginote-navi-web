...
/* 全期間固定（フラット35）の当月金利(%)。2026-07-29 追加。
 * /loan の計算ツールと記事で「固定に切り替えたらいくらか」を出すために使う。
 * フラット35の金利は毎月改定されるため、**毎月ここだけを手動更新する**。
 * 2026年7月時点：3.14%（返済期間21年以上・融資率9割以下の最も多い金利）。
 * ※ /shisan の診断は参照しない（既存の計算・表示に影響はない）。 */
export const FLAT35_RATE = 3.14;
